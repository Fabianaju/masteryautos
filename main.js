document.addEventListener('DOMContentLoaded', function() {
    ...
    var sound = document.getElementById('engine-sound');

    logo.addEventListener('click', function(event) {
        event.preventDefault(); // Evita o redirecionamento do link
        if (!sound.paused) {
            sound.pause(); // Pausa o som se ele estiver tocando
            sound.currentTime = 0; // Volta ao início do som
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    logo.addEventListener('click', function(event) {
        event.preventDefault(); // Evita o redirecionamento do link
        if (sound.paused) {
            sound.play(); // Reproduz o som se estiver pausado
        } else {
            sound.pause(); // Pausa o som se estiver tocando
            sound.currentTime = 0; // Volta ao início do som
        }
    });
});
